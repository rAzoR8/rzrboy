...
namespace dbg.ui
{
    ...
    public class Renderer : IDisposable
    {
        ...
        /// </summary>
        public Renderer(GraphicsDevice gd, OutputDescription outputDescription, int width, int height)
        {
            Device = gd;
            _windowWidth = width;
        ...
        public void CreateDeviceResources(OutputDescription outputDescription)
        {
            ...
            _vertexShader = factory.CreateShader(new ShaderDescription(ShaderStages.Vertex, vertexShaderBytes, Device.BackendType == GraphicsBackend.Metal ? "VS" : "main"));
            _fragmentShader = factory.CreateShader(new ShaderDescription(ShaderStages.Fragment, fragmentShaderBytes, Device.BackendType == GraphicsBackend.Metal ? "FS" : "main"));
        ...
        {
            switch (Device.ResourceFactory.BackendType)
            {
        ...
        public void RecreateFontDeviceTexture()
        {
            ...
            _fontTexture = Device.ResourceFactory.CreateTexture(TextureDescription.Texture2D(
                (uint)width,
                (uint)height,
                1,
                1,
            ...
            Device.UpdateTexture(
                ...
                0,
                0);
            _fontTextureView = Device.ResourceFactory.CreateTextureView(_fontTexture);

            io.Fonts.ClearTexData();

			_fontTextureResourceSet?.Dispose();
			_fontTextureResourceSet = Device.ResourceFactory.CreateResourceSet( new ResourceSetDescription( _textureLayout, _fontTextureView ) );
		}
        ...
        private void RenderImDrawData(ImDrawDataPtr draw_data, CommandList cl)
        {
            ...
            {
                Device.DisposeWhenIdle(_vertexBuffer);
                _vertexBuffer = Device.ResourceFactory.CreateBuffer(new BufferDescription((uint)(totalVBSize * 1.5f), BufferUsage.VertexBuffer | BufferUsage.Dynamic));
            }

            uint totalIBSize = (uint)(draw_data.TotalIdxCount * sizeof(ushort));
            if (totalIBSize > _indexBuffer.SizeInBytes)
            {
                Device.DisposeWhenIdle(_indexBuffer);
                _indexBuffer = Device.ResourceFactory.CreateBuffer(new BufferDescription((uint)(totalIBSize * 1.5f), BufferUsage.IndexBuffer | BufferUsage.Dynamic));
            }
            ...
            Device.UpdateBuffer(_projMatrixBuffer, 0, ref mvp);
